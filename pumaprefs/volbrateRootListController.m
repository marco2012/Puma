...
@implementation volbrateRootListController
	...
	-(void)viewDidLoad {
		...
		UIImage *iconBar = [[UIImage alloc] initWithContentsOfFile:@"/Library/PreferenceBundles/pumaprefs.bundle/navlogo.png"];
	...
	-(void)resetSettings {
		...
		UIAlertAction * confirmAction = [UIAlertAction actionWithTitle:@"Confirm" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * action) {
			NSError * error = nil;
			[[NSFileManager defaultManager] removeItemAtPath:@"/User/Library/Preferences/com.lacertosusrepo.pumaprefs.plist" error:&error];
			if(!error) {
				CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), CFSTR("me.vikings.puma-respring"), nil, nil, true);
			} else {
				NSLog(@"error - %@", error);
				UIAlertController * failedAlert = [UIAlertController alertControllerWithTitle:@"Puma"
	...
	-(void)sendTestVibtration	{
		CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), CFSTR("me.vikings.puma-testvibration"), nil, nil, true);
	}

	-(void)twitter {
		[[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"https://twitter.com/marco2012"] options:@{} completionHandler:nil];
	}

	-(void)paypal	{
		[[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"https://www.paypal.me/marco2012"] options:@{} completionHandler:nil];
	}
